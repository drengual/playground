...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-slate-900 selection:bg-blue-100 selection:text-blue-900">
      ...
      {/* 1. HERO SECTION */}
            <section id="hero" className="pt-24 md:pt-40 pb-16 px-4 sm:px-6 bg-linear-to-b from-white to-slate-50">
          <div className="max-w-7xl mx-auto grid lg:grid-cols-12 gap-12 items-center">
            
            {/* Left: Content */}
            <div className="lg:col-span-7 space-y-6 md:space-y-8">
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-blue-50 text-blue-700 rounded-full text-xs font-bold tracking-widest uppercase">
                <span className="relative flex h-2 w-2">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-blue-400 opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-2 w-2 bg-blue-600"></span>
                </span>
                Trusted Dental Clinic in Makati
              </div>
              ...
              <p className="text-lg md:text-xl text-slate-500 max-w-xl leading-relaxed">
                Experience stress-free dentistry with Makati's leading team. Request your appointment online and get confirmed in minutes.
              </p>
              
              <div className="flex flex-col sm:flex-row gap-4">
                <button 
                  onClick={() => scrollToSection('contact-form')} 
                  className="md:hidden bg-blue-600 text-white px-8 py-4 rounded-2xl font-bold text-lg hover:bg-slate-900 transition-all flex items-center justify-center gap-3 shadow-xl shadow-blue-100"
                >
                  Book Appointment <ArrowRight size={20} />
                </button>
                <a href="tel:09123456789" className="bg-white border-2 border-slate-200 text-slate-900 px-8 py-4 rounded-2xl font-bold text-lg hover:bg-slate-50 transition-all flex items-center justify-center gap-3">
                  <Phone size={20} className="text-blue-600" /> 0912 345 6789
                </a>
              </div>
            </div>

            {/* Right: Embedded Form (Desktop) */}
            <div id="contact-form" className="lg:col-span-5 scroll-mt-24">
              <div className="bg-white p-6 md:p-8 rounded-[2.5rem] shadow-2xl shadow-blue-100 border border-slate-100">
                <div className="mb-6">
                  <h2 className="text-2xl font-black text-slate-900">Request a Slot</h2>
                  <p className="text-slate-500 text-sm">Our team will call to confirm your schedule.</p>
                </div>
                ...
                >
                  <div className="grid gap-4">
                    <input required type="text" placeholder="Full Name" className="w-full p-4 bg-slate-50 rounded-xl border-none ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-600 outline-none transition-all" />
                    <input required type="tel" placeholder="Mobile Number" className="w-full p-4 bg-slate-50 rounded-xl border-none ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-600 outline-none transition-all" />
                    <select required className="w-full p-4 bg-slate-50 rounded-xl border-none ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-600 outline-none appearance-none">
                  ...
                  <button 
                    ...
                    className={`w-full py-4 rounded-xl font-black text-lg shadow-lg transition-all ${
                      formStatus === 'success' ? 'bg-green-500 text-white' : 'bg-blue-600 text-white hover:bg-slate-900 shadow-blue-200'
                    }`}
                  >
                    {formStatus === 'success' ? 'REQUEST SENT!' : 'CONFIRM REQUEST'}
                  </button>
